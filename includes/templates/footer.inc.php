    <footer class="site-footer">
        <div class="contenedor grid">
            <div class="footer-informacion columnas-3">
                <h4>Sobre nosotros </h4>
                <p>
                    Lorem ipsum dolor, sit amet consectetur adipisicing elit. Et blanditiis provident ea? Laborum
                    possimus
                    voluptates dolorem ut, eos atque nobis dignissimos nisi quaerat dicta explicabo perferendis
                    obcaecati,
                    neque,
                    qui enim.
                </p>
            </div>
            <div class=" columnas-3">
                <h4>Comunidad </h4>
                <nav class="navegacion-footer">
                        <a href="nosotros.php">Sobre nosotros</a>
                        <a href="ayuda.php">Centro de ayuda</a>
                        <a href="blog.php">Blog</a>
                    </nav>
            </div>
            <div class=" columnas-3">
                <h4>Transfer</h4>
                <nav class="navegacion-footer">
                    <a href="index.php">Home</a>
                    <a href="musica.php">Musica</a>
                    <a href="fotos.php">Fotos</a>
                    <a href="videos.php">Videos</a>
                    <a href="registro.php">Registrate </a>
                    <a href="login.php">Inicia Sesion</a>
                </nav>
            </div>
            <div class="menu columnas-3">
                <h4>Redes Sociales </h4>
                <nav class="redes-sociales">
                    <a href="https://www.facebook.com/Transferonline-2329476567379056/?modal=admin_todo_tour" target="blank"><i class="fab fa-facebook"></i></a>
                    <a href="https://www.twitter.com" target="blank"><i class="fab fa-twitter"></i></a>
                    <a href="https://www.instagram.com" target="blank"><i class="fab fa-instagram"></i></a>
                </nav>
                <p class="copyright">Todos los derechos Reservados IVAN BOLAÑOS 2019</p>
            </div>
        </div>
    </footer>



</body>

